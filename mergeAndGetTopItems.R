...
filesToMerge <- function(day0, today) {
  ...
  if (daysAfter0 == 0){
    ...
    filesToMergeAfter0 <- sprintf("%s%04d", "004-dailyTop10/top10day",  0:(daysAfter0-1))
  }
  
  filesToMergeBefore0 <- sprintf("%s%04d", "004-dailyTop10/top10day", 1000 + 1:abs(daysBefore0))
...
readAllCategoryItems <- function() {
  con <- file("006-allCategoryItems/allCategoryItemDB.txt", "r")
...
updateCount <- function(df) {
  ...
  if(is.null(df2)){
    df2 <- data.frame(cat_1 = character(0), cat_2 = character(0), cat_3 = character(), item_id = character(0), count = numeric(0))
  } else {
...
getTopItemsInCategory <- function(df){
  library(dplyr)
  df2 <- df %>% group_by(cat_1, cat_2, cat_3, item_id) %>%
